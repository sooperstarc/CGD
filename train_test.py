...
from utils_test import recall, LabelSmoothingCrossEntropyLoss, BatchHardTripletLoss, ImageReader, MPerClassSampler, custom_precision_top_k
...
def test(net, recall_ids):
    ...
    with torch.no_grad():
        ...
        # compute metrics
        acc_list = custom_precision_top_k(eval_dict['test']['features'], test_data_set.labels, recall_ids)
            
    desc = 'Test Epoch {}/{} '.format(epoch, num_epochs)
    for index, rank_id in enumerate(recall_ids):
        desc += 'Precision@{}:{:.2f}% '.format(rank_id, acc_list[index] * 100)
        results['test_precision@{}'.format(rank_id)].append(acc_list[index] * 100)
    print(desc)
    return acc_list[0]
